...
describe ActiveAdminHelper do
  describe '#folder_collection' do
    it 'should return an array with the correct data' do
      ...
      expect(described_class.folder_collection).to eq([[root_folder.name, root_folder.id]])

      folder1 = create(:folder, :name => 'Abc')
      folder2 = create(:folder, :name => 'Zyx')
      expect(described_class.folder_collection).to eq(
        [
      ...
      expect(described_class.folder_collection(:include_root => false)).to eq(
        [
          [folder1.name, folder1.id],
          [folder2.name, folder2.id]
        ]
  ...
  describe '#model_collection' do
    it 'should apply a formatter if given' do
      ...
      expect(described_class.model_collection(Folder, :formatter => :upcase)).to eq(
        [
    ...
    it 'should apply the block if given' do
      ...
      expect(described_class.model_collection(Folder) { |f| f.id + 5 }).to eq(
        [
  ...
  describe '#unique_field_collection' do
    it 'should return a list of unique value for that given attribute' do
      root_folder = create(:root_folder)
      create(:folder, :parent_folder => root_folder)
      create(:folder, :parent_folder => root_folder)

      expect(described_class.unique_field_collection(Folder, :parent_folder_id) { |_f| 'A' }).to eq(
  ...
  describe '#generate_id_union_query' do
    it 'should perform a UNION query to get teh data needed' do
